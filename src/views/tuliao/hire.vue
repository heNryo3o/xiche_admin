<template>
  <div class="app-container">
    <el-row class="filter-container" :gutter="10">
      <el-col :sm="4">
        <el-input v-model="listQuery.company" size="medium" prefix-icon="el-icon-search" placeholder="输入企业名称搜索" clearable
    ...
    <div class="table-container">
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="预算" width="150">
          <template slot-scope="{row}">
            <span>{{ row.budget_type_name }}</span>
          </template>
        </el-table-column>

        <el-table-column label="期望入职时间" width="150">
          <template slot-scope="{row}">
        ...
        <el-table-column label="操作" fixed="right" min-width="230" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <el-button v-waves v-permission="['hire/edit-hire']" type="primary" size="mini" @click="handleUpdate(row)">
              编辑
            </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" width="700px" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 550px; margin-left:50px;">
        ...
        <el-form-item label="发布企业">
          <el-input v-model="temp.company" placeholder="请填写发布企业名称" />
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      handleChangeStatus(row, status) {
        var message = status === 1 ? '招人信息启用成功' : '招人信息禁用成功'
        changeStatus({
          id: row.id,
          status: status
        }).then(response => {
