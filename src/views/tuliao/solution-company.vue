<template>
  <div class="app-container">
    <el-row class="filter-container" :gutter="10">
      <el-col :sm="4">
        <el-input v-model="listQuery.company_name" size="medium" prefix-icon="el-icon-search" placeholder="输入专家名称搜索" clearable
      ...
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        <el-table-column label="专家名" width="200">
          <template slot-scope="{row}">
            <span class="link-type" @click="showInfo(row.id)">{{ row.company_name }}</span>
          </template>
        </el-table-column>
    ...
    <solution-company :id="itemId" :info-visible.sync="infoVisible" />
...
<script>
  ...
  import SolutionCompany from '@/components/Solution/SolutionCompany'
