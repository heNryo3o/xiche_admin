  ...
  <div class="app-container">
      ...
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="认证状态状态" width="120">
          <template slot-scope="{row}">
            ...
            <el-popover v-else-if="row.status == 3" placement="bottom" title="审核未通过原因：" width="200" trigger="hover"
              :content="row.msg">
        ...
        <el-table-column label="操作" fixed="right" min-width="180" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <el-button v-if="row.status == 1" v-waves v-permission="['role/change-status']" size="mini" type="success"
    ...
    <el-dialog title="查看认证信息" width="900px" :visible.sync="dialogVisible">
      <div class="preview-container">
        ...
        <p>
          银行卡号:  <b>{{ temp.bank_card }}</b>
        </p>
        <p>
          银行名称: {{ temp.bank_name }}
        </p>
        <p>
          银行卡类型: {{ temp.bank_type }}
        </p>
        <p>
          预留手机号: {{ temp.bank_mobile }}
        </p>
        <p>
          姓名: {{ temp.bank_nickname }}
        </p>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        workerWithdraw(this.listQuery).then(response => {
