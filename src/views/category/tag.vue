    ...
    <div class="table-container">
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="标签类型" width="120">
          <template slot-scope="{row}">
            <span>{{ row.type === 1 ? '成分标签' : '工艺标签' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" width="700px" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 550px; margin-left:50px;">
        ...
        <el-form-item label="关联分类">
          <div class="block">
            <el-cascader v-model="temp.category" :options="categoryOptions" :props="{ checkStrictly: true, multiple: true }" clearable />
          </div>
        </el-form-item>

        <el-form-item label="标签类型">
          <div>
            <el-radio v-model="temp.type" label="1" border size="medium">成分标签</el-radio>
            <el-radio v-model="temp.type" label="2" border size="medium">工艺标签</el-radio>
          </div>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.temp.type = this.temp.type === 1 ? '1' : '2'
